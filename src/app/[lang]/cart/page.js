...
// 1. IMPORTAMOS 'use' PARA NEXT.JS 15 Y HOOKS DE REACT
...
// =========================================================
// 1. GENERADOR DE PLANTILLA HTML PARA ADMIN (EMPRESA)
// =========================================================
...
export default function CheckoutPage({ params }) {
  ...
  const procesarConfirmacion = async (detallesPago = null, metodoOverride = null) => {
    ...
    try {
      ...
      alert(isEs ? "Hubo un error al procesar tu reserva. Por favor intenta nuevamente." : "There was an error processing your booking. Please try again.");
    } finally {
  ...
  const t = {
    ...
    arrivalFlight: isEs ? "Vuelo de Llegada al Aeropuerto (SJD)" : "Arrival Flight (SJD Airport)",
  ...
  const renderStepper = () => (
    <div className="flex items-center justify-between w-full max-w-2xl mx-auto mb-12 px-4 relative">
      ...
      {[{ num: 1, label: t.step1, active: true }, 
        { num: 2, label: t.step2, active: step >= 2 }, 
        { num: 3, label: t.step3, active: step >= 3 }, 
        { num: 4, label: t.step4, active: step >= 4 }].map((s, i) => (
                ...
                <div className="flex-1 space-y-8">
                  <div className="bg-white border border-slate-200/60 rounded-[2rem] p-6 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2 tracking-tight">
                      <User className="text-blue-600" size={24} /> {t.titularTitle}
                    </h2>
                  ...
                  <div className="bg-white border border-slate-200/60 rounded-[2rem] p-6 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2 tracking-tight">
                      <Plane className="text-blue-600" size={24} /> {t.flightTitle}
                    </h2>
          ...
          {step === 4 && (
            <div className="animate-fade-in w-full max-w-4xl mx-auto space-y-8">
              <div className="bg-emerald-500 rounded-[2rem] p-10 md:p-16 text-center text-white shadow-[0_20px_50px_rgba(16,185,129,0.2)] flex flex-col items-center">
                ...
                <div className="bg-white/20 backdrop-blur-sm border border-white/30 rounded-2xl py-4 px-8 inline-block">
                  <p className="text-3xl font-black tracking-widest">{confirmNumber}</p>
                </div>
              </div>
