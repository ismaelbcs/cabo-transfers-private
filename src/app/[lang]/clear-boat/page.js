// src/app/[lang]/clear-boat/page.js
'use client';

import React, { useMemo, useEffect, use } from 'react';
import { ArrowLeft, ImageIcon, CheckCircle, Check, Info, Clock, Calendar, Users, AlertCircle, MapPin } from 'lucide-react';
import { useBooking } from '../../../context/BookingContext';
import { useCart } from '../../../context/CartContext';
import { useRouter } from 'next/navigation';
import { toursData } from '../../../data/seoData';

export default function ClearBoatPage({ params }) {
  ...
  // Forzar la carga del tour "Bote Transparente al Arco" ('arco')
  const tr = toursData.find(t => t.id === 'arco');

  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });

    // Inicializamos el ID de este tour en la reserva, con mínimo 2 pasajeros
    setReserva(prev => ({
      ...prev,
      tourId: 'arco',
      pasajeros: Math.max(prev.pasajeros || 1, 2)
    }));
    setServicioSeleccionado('tours');
    setImagenTourDestacada('B7.webp');
    setPaso(2);
  }, [setReserva, setServicioSeleccionado, setImagenTourDestacada, setPaso]);
