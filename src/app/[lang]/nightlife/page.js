...
export default function NightlifePage({ params }) {
  ...
  const [nightlifeHora, setNightlifeHora] = useState(''); // Pick-up
  const [nightlifeHoraRegreso, setNightlifeHoraRegreso] = useState(''); // Return

  const matrizPreciosNightlife = {
    'sjc': { '1': 200, '2': 140, '3': 170, '4': 250 },
    'corredor': { '1': 250, '2': 120, '3': 140, '4': 220 },
    'csl': { '1': 300, '2': 140, '3': 120, '4': 220 },
    'pacifico': { '1': 380, '2': 220, '3': 220, '4': 180 }
  };

  const recargoNightlife = { '1-4': 0, '5-8': 30, '9-10': 45 };
  ...
  const granTotalNightlife = totalPrecioNightlife > 0 ? totalPrecioNightlife : 0;

  const calculoHorasNightlife = useMemo(() => {
    if (!nightlifeHora || !nightlifeHoraRegreso) return { error: false, diffMin: 0 };
    const [sH, sM] = nightlifeHora.split(':').map(Number);
  ...
  }, [nightlifeHora, nightlifeHoraRegreso]);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 md:p-8 animate-fade-in relative">
          <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded-xl p-4 text-sm font-semibold mb-6 flex items-start gap-3">
            ...
            <p>{lang === 'es' ? 'El servicio de traslado Nightlife contempla un ' : 'The Nightlife transfer service includes a '}<strong>{lang === 'es' ? 'máximo de 4 horas' : 'maximum of 4 hours'}</strong>{lang === 'es' ? ' de espera en la fiesta (más 1 hora de traslado).' : ' of wait time at the party (plus 1 hour of transit).'}</p>
          </div>

          <div className="mb-8">
            <h1 className="text-2xl font-extrabold text-gray-900">{lang === 'es' ? 'Reserva de Transporte para Nightlife' : 'Nightlife Transportation Booking'}</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col relative">
              ...
              {mostrarDropdownNightlife && (
                <ul className="absolute z-50 w-full bg-white border border-gray-200 rounded-lg shadow-xl mt-1 top-[76px] max-h-60 overflow-y-auto">
                  {catalogoHoteles.filter(h => (busquedaNightlifeOrigen || '').toLowerCase().split(' ').every(w => h.nombre.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toLowerCase().includes(w.normalize('NFD').replace(/[\u0300-\u036f]/g, "")))).slice(0, 30).map((hotel, idx) => (
                    <li key={`${hotel.id}-${idx}`} onMouseDown={() => { const zona = hotel.zonaId || hotel.zona || "1"; setNightlifeOrigen(`${hotel.nombre}|${zona}`); setBusquedaNightlifeOrigen(`${hotel.nombre} (Zona ${zona})`); setMostrarDropdownNightlife(false); }} className="p-3 hover:bg-blue-50 cursor-pointer text-gray-700 border-b border-gray-100 last:border-0 transition flex justify-between">
            ...
            <div className="flex flex-col">
              <label className="text-sm font-semibold text-gray-700 mb-2">{lang === 'es' ? '¿En qué área está la fiesta?' : 'Party area?'}</label>
            ...
            <div className="flex flex-col">
              <label className="text-sm font-semibold text-gray-700 mb-2">{lang === 'es' ? 'Nombre exacto del lugar' : 'Exact Place Name'}</label>
              <input type="text" placeholder={lang === 'es' ? 'Ej. Mandala, Squid Roe, etc.' : 'E.g. Mandala, Squid Roe, etc.'} value={nightlifeLugarNombre} onChange={(e) => setNightlifeLugarNombre(e.target.value)} className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-900 outline-none text-gray-700" />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <div className="relative">
                ...
                <input type="date" value={nightlifeFecha} onChange={(e) => setNightlifeFecha(e.target.value)} className="w-full pl-10 pr-3 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-900 outline-none text-gray-700" />
              </div>
            </div>
            ...
            <div className="flex flex-col md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col">
                ...
                <input type="time" value={nightlifeHora} onChange={(e) => { setNightlifeHora(e.target.value); if (!e.target.value) setNightlifeHoraRegreso(''); }} className="w-full p-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-900 outline-none text-gray-700" />
              </div>
              <div className="flex flex-col relative">
                <label className="text-sm font-semibold text-gray-700 mb-2">{lang === 'es' ? 'Hora de recogida de la fiesta (regreso)' : 'Pick-up time (Party return)'}</label>
                <input 
                  type="time" 
                  value={nightlifeHoraRegreso} 
                  onChange={(e) => setNightlifeHoraRegreso(e.target.value)} 
                  disabled={!nightlifeHora}
                  className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-blue-900 outline-none text-gray-700 disabled:bg-gray-100 disabled:cursor-not-allowed ${calculoHorasNightlife.error ? 'border-red-500 bg-red-50 focus:ring-red-500' : 'border-gray-200'}`} 
                />
            ...
            <div className="md:col-span-2 mt-4 bg-slate-900 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between text-white shadow-md">
              <div className="mb-4 md:mb-0 text-center md:text-left">
                ...
                <h3 className="text-4xl md:text-5xl font-bold tracking-tight">
                  {granTotalNightlife > 0 ? `$${granTotalNightlife.toFixed(2)}` : '---'}
                </h3>
                {granTotalNightlife === 0 ? (
                  <p className="text-xs text-yellow-400 mt-2 font-medium">{lang === 'es' ? 'Selecciona área de la fiesta para calcular' : 'Select party area to calculate'}</p>
                ) : (
                  <div className="mt-2">
              ...
              <button
                disabled={!nightlifeOrigen || !nightlifeDestino || !nightlifeLugarNombre || !nightlifeFecha || !nightlifeHora || !nightlifeHoraRegreso || calculoHorasNightlife.error}
                onClick={() => {
                  agregarAlCombo({
                    id: `nightlife-${Date.now()}`,
                    titulo: lang === 'es' ? `Nightlife: ${nightlifeLugarNombre}` : `Nightlife: ${nightlifeLugarNombre}`,
                    subtitulo: `Fecha: ${nightlifeFecha} | Pax: ${nightlifePax} | ${nightlifeHora} - ${nightlifeHoraRegreso}`,
                    precio: granTotalNightlife,
                    tipoEspecial: 'nightlife',
                    config: { nightlifeOrigen, nightlifeDestino, nightlifePax, nightlifeFecha, nightlifeLugarNombre, nightlifeHora, nightlifeHoraRegreso }
                  });
