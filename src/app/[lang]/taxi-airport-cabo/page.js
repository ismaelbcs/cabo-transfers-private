...
export async function generateMetadata({ params }) {
  ...
  if (lang === 'es') return {
    title: 'Alternativa a Taxi Airport Cabo | Transporte Privado VIP',
    description: '¿Buscas un Taxi Airport Cabo? Evita los taxímetros y largas filas. Reserva nuestro transporte privado con tarifa fija garantizada.',
    keywords: 'taxi airport cabo, taxi aeropuerto cabo, precio taxi aeropuerto los cabos, alternativa taxi SJD',
  };
  return {
    title: 'Taxi Airport Cabo Alternative | Flat-Rate Private Transfers',
    description: 'Looking for a Taxi Airport Cabo? Skip the running meters and long lines. Book our private transportation with a guaranteed flat rate.',
    keywords: 'taxi airport cabo, cabo airport taxi rates, alternative to taxi cabo, flat rate SJD transfers',
  };
}
        ...
        <section className="mb-20">
          ...
          <div className="prose prose-lg text-neutral-600 prose-p:leading-relaxed max-w-none">
            ...
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-neutral-900 mb-6">
              {lang === 'es'
                ? 'Mucho Más que un Simple Taxi'
                : 'Much More Than Just a Regular Taxi'}
            </h2>
            <div className="prose prose-lg text-neutral-600 prose-p:leading-relaxed max-w-none">
              <p className="mb-6">
                {lang === 'es' ? (
                  <>Aunque buscar un <strong>taxi airport cabo</strong> al llegar puede parecer fácil, a menudo implica tiempos de espera, vehículos pequeños y tarifas dinámicas por el tráfico hacia la zona hotelera.</>
                ) : (
                  <>While looking for a <strong>taxi airport cabo</strong> upon arrival might seem easy, it often involves wait times, small vehicles, and unpredictable metered rates due to traffic toward the hotel zone.</>
                )}
              </p>
              <p className="mb-6">
                {lang === 'es' ? (
                  <>En lugar de depender de un <strong>taxi airport cabo</strong> regular, te ofrecemos una experiencia premium: SUV de lujo, precios fijos confirmados previamente y choferes dedicados que hablan inglés.</>
                ) : (
                  <>Instead of relying on a standard <strong>taxi airport cabo</strong>, we offer a premium experience: luxury SUVs, pre-confirmed flat rates, and dedicated English-speaking drivers.</>
                )}
              </p>
              <p>
                {lang === 'es' ? (
                  <>Sube el nivel de tu viaje. Un servicio privado te brinda la confianza y el espacio que un clásico <strong>taxi airport cabo</strong> simplemente no está diseñado para ofrecer a familias o grupos.</>
                ) : (
                  <>Upgrade your journey. A private service gives you the reliability and space that a classic <strong>taxi airport cabo</strong> is simply not designed to offer for families or groups.</>
                )}
              </p>
            </div>
          </div>
