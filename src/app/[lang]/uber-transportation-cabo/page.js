...
export async function generateMetadata({ params }) {
  ...
  if (lang === 'es') return {
    title: 'Mejor que Uber Transportation Cabo | Traslados Privados SJD',
    description: 'La opción superior a Uber Transportation Cabo. Choferes bilingües esperándote con letrero dentro del aeropuerto. Sin caminatas, sin cancelaciones.',
    keywords: 'uber transportation cabo, transporte los cabos vs uber, SJD transfers',
  };
  return {
    title: 'Better Than Uber Transportation Cabo | Private SJD Transfers',
    description: 'The superior choice to Uber Transportation Cabo. Bilingual drivers waiting inside the airport with a sign. No walking off-property, no cancellations.',
    keywords: 'uber transportation cabo, private driver cabo vs uber, SJD airport authorized transportation',
  };
}
        ...
        <section className="mb-20">
          ...
          <div className="prose prose-lg text-neutral-600 prose-p:leading-relaxed max-w-none">
            ...
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight text-neutral-900 mb-6">
              {lang === 'es'
                ? 'Tu Transporte Premium y Oficial'
                : 'Your Premium and Official Ride'}
            </h2>
            <div className="prose prose-lg text-neutral-600 prose-p:leading-relaxed max-w-none">
              <p className="mb-6">
                {lang === 'es' ? (
                  <>Aunque buscar opciones de <strong>uber transportation cabo</strong> es común en la ciudad, cuando se trata de traslados desde el aeropuerto, necesitas un servicio autorizado, puntual y sin tiempos de espera impredecibles.</>
                ) : (
                  <>While searching for <strong>uber transportation cabo</strong> is common in town, when it comes to airport transfers, you need an authorized, punctual service without unpredictable wait times.</>
                )}
              </p>
              <p className="mb-6">
                {lang === 'es' ? (
                  <>No dejes tu traslado al azar ni a la disponibilidad de un vehículo pequeño de <strong>uber transportation cabo</strong>. Disfruta de la garantía de una camioneta de lujo, perfectamente climatizada y lista exclusivamente para ti.</>
                ) : (
                  <>Do not leave your transfer to chance or the availability of a compact <strong>uber transportation cabo</strong> vehicle. Enjoy the guarantee of a luxury SUV, perfectly air-conditioned and ready exclusively for you.</>
                )}
              </p>
              <p>
                {lang === 'es' ? (
                  <>Con precios fijos y transparentes, superamos la experiencia de cualquier <strong>uber transportation cabo</strong>. Te brindamos asistencia con el equipaje y una ruta directa y relajante a tu destino final.</>
                ) : (
                  <>With flat, transparent pricing, we easily surpass the experience of any <strong>uber transportation cabo</strong>. We provide luggage assistance and a direct, relaxing route to your final destination.</>
                )}
              </p>
            </div>
          </div>
